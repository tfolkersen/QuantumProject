...
quadraticProblem = dimod.make_quadratic(problem, 500, dimod.BINARY)
...
solver = ExactSolver()
sampleset = solver.sample(quadraticProblem).lowest()


print("Samples: " + str(len(sampleset.samples())))
for s in sampleset.samples():
    p = "1" + str(s["p2"]) + str(s["p1"]) + "1"
    q = "1" + str(s["q2"]) + str(s["q1"]) + "1"
    print(p + " " + q)
